...
describe("R2 causal organism substrate", () => {
  ...
  it("holds World event and observation constant while private history changes", () => {
    const challenge = challenges[0]!;
    const left = challenge.variants[0]!.episodeFrame;
    const right = challenge.variants[1]!.episodeFrame;
  ...
  it("can keep identical World truth hidden from an actor with no observation path", () => {
    ...
    const seen = challenge.variants[0]!.episodeFrame;
    const unseen = challenge.variants[1]!.episodeFrame;
  ...
  it("keeps identical observed motion compatible with different World provenance", () => {
    ...
    const ownerOrigin = challenge.variants[0]!.episodeFrame;
    const externalOrigin = challenge.variants[1]!.episodeFrame;
  ...
  it("allows World-truth authority only for an explicit perception oracle", () => {
    const frame = challenges[1]!.variants[0]!.episodeFrame.privateByActor[
      "resident:mira"
    ]!;
  ...
  it("requires post-perception oracle claims to cite actor-private evidence", () => {
    const frame = challenges[0]!.variants[0]!.episodeFrame.privateByActor[
      "resident:mira"
    ]!;
